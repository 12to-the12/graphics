...
def orthogonal(a, b):
    ...
    if np.all(a == b):
        assert False, f"no orthogonal between {a} and {b}"
    if not np.any(np.cross(a, b)):
        assert False, f"no orthogonal between diametrically opposed vectors"
...
def quaternion(vectors, x, transpose=False):
    ...
    t = vectors * x
    if transpose:
        # transposes each vector operation, leaving the list of them in the right order
        t = np.transpose(t, (0, 2, 1))

    assert t.shape[1:] == (
        4,
        4,
    ), "the t array is not shaped correctly, it should be a list of 4x4 matrices"
...
def arbitrary_axis_rotation(points, rotation_axis, degrees):  # vector_array
    ...
    assert (
        points.ndim == 2
    ), "the arbitrary_axis_rotation function operates on a 2 dimensional list"
    assert (
        points.shape[1] == 3
    ), "the arbitrary_axis_rotation function takes a LIST of vectors"
    ...
    q = np.array(q)
    # q prime is an inverted q, with the magnitude left alone
    qprime = (q[0], q[1] * -1, q[2] * -1, q[3] * -1)
...
if __name__ == "__main__":
    ...
    rotation_axis = np.array([1.0, 1.0, 1.0])
    rotation_axis = norm(rotation_axis)
    theta = 240.0  # theta 240  725, returns 0257
    # theta == np.float64(theta)
    vectors = np.array([[7.0, 2.0, 5.0]])
    ...
    # ten million seems to be the scalable limit
